...
skip_samples_file="/local/tsaid/trabajo/scripts/tumoural_2019.txt"  # File containing samples to skip
...
# Function to check if a sample is in the skip list
skip_sample() {
  local sample=$1
  if grep -Fxq "$sample" "$skip_samples_file"; then
    echo "Sample $sample is in the skip list, skipping conversion."
    return 0
  else
    return 1
  fi
...
for idat_dir in $base_path_to_idat_folder/*/; do
  ...
  # Check if the sample should be skipped
  if skip_sample "$output_idat"; then
    continue  # Skip this sample if it's in the skip list
  fi
